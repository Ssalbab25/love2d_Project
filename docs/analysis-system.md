...
목적: 체스닷컴(Chess.com)과 유사한 게임 분석 환경을 제공하여 플레이어의 학습을 돕고 게임의 시각적 재미를 배가시킨다.

핵심 기능:
- 실시간 형세 평가 바 (Evaluation Bar): 현재 보드 상황을 점수화하여 흑/백 중 누가 얼마나 유리한지 화면 측면에 그래픽으로 표시.
- 추천 수 화살표 (Hint Arrow): 시스템이 계산한 최적의 다음 수(Best Move)를 보드 위에 반투명 화살표로 시각화.
