...
function test_AIMode.run()
    ...
    -- 5. vs AI 모드에서 타이머 감소 검증 (기획안 기준으로 정지 상태 확인)
    assert(game.currentTurn == "white", "게임 시작 후 턴은 'white'여야 합니다.")
    local prevWhiteTime = game.whiteTime
    game:update(1.0)
    assert(game.whiteTime == prevWhiteTime, "vs AI 모드에서는 White 턴일 때 제한 시간이 정지 상태(Pause)여야 합니다.")
    ...
    assert(game.blackTime == prevBlackTime, "vs AI 모드에서는 Black 턴일 때 제한 시간이 정지 상태(Pause)여야 합니다.")
