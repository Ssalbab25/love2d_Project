-- Queen.lua: 퀸 기물의 행마 및 상태 클래스
-- SOLID: 
-- - OCP (개방-폐쇄 원칙): Piece 추상 클래스를 상속받아 기존 코드를 수정하지 않고 Queen 고유의 getValidMoves를 확장 구현합니다.
-- - LSP (리스코프 치환 원칙): 부모 Piece와 동일하게 getValidMoves가 { {row = r, col = c}, ... } 구조의 배열을 일관되게 반환합니다.
-- - SRP (단일 책임 원칙): 오직 퀸 기물의 고유 속성 및 이동 규칙 계산의 책임만 가집니다.

local Piece = require "src.pieces.Piece"
local Queen = Piece:extend()

function Queen:new(color)
    Queen.super.new(self, color, "queen")
end
...
-- 반환값: { {row = r, col = c}, ... } 형태
function Queen:getValidMoves(board, currentPos)
    local moves = {}
    local r = currentPos.row
    local c = currentPos.col
    
    -- 8방향 정의 (상하좌우 및 대각선 4방향)
    local directions = {
        -- 상하좌우
        { dr = -1, dc = 0 },
        { dr = 1,  dc = 0 },
